...
int main(int argc, char const *argv[]) {
    ...
    widgetSetColor(&testStyle.background, 128, 20, 20, 255);
    widgetSetColor(&testStyle.active, 255, 20, 20, 255);

    widget_button* testButton = newWidgetButton("test", &testStyle, 20, 20);

    widget_button* testButton2 = newWidgetButton("test", &testStyle, 20, 50);
	...
	do{
        ...
        if(isWidgetButtonPressed(testButton) == SDL_BUTTON_LEFT){
            quit = 1;
        }

        SDL_RenderClear(renderer);
        drawWidgetButton(renderer, testButton, 2, 2);
        drawWidgetButton(renderer, testButton2, 50, 50);

        SDL_RenderPresent(renderer);

    }while(!quit);
    freeWidgetButton(testButton);
    freeWidgetButton(testButton2);
    return 0;
}
